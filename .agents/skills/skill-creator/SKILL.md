...
| Aspect | Skills (`.Codex/skills/`) | Commands (`.Codex/commands/`) |
|--------|---------------------------|-------------------------------|
...
doesn't already have. Challenge each piece: "Does Codex really need this?"
...
description:
  ...
  Codex reads this to decide when to load the skill. Include specific
...
3. **References** - As needed by Codex (unlimited)
...
Codex reads reference files only when needed.
...
- "What does Codex NOT already know that this should teach?"
...
3. Verify Codex uses the skill appropriately
...
```bash
touch .Codex/commands/my-command.md
```
...
- Obvious information Codex already knows
- Overly detailed explanations (use examples instead)
- User-facing documentation (skills are for Codex)
